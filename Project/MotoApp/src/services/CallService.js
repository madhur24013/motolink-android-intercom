...
export const CallService = {
  setSignalingChannel: (channel, device) => {
    ...
    LogsService.add(
      ...
      `${channel?.transport || 'unknown'}:${
        device?.address || device?.id || 'unknown'
      }`,
      'CHANNEL',
    );
  },
  ...
  initiateCall: async (device, onCallStateChange) => {
    ...
    AudioService.createPeerConnection(
      WEBRTC_TRICKLE_ICE
        ? candidate =>
            CallService._sendSignal({type: 'ice', sessionId, candidate})
        : null,
    ...
    await CallService._sendSignal({
      ...
      callerId: 'self',
      callerName:
        Storage.getRole() === 'pillion' ? 'MotoLink Pillion' : 'MotoLink Rider',
      role: Storage.getRole() || 'unknown',
      offer,
      codec: 'OPUS',
      transport:
        signalingChannel?.transport ||
        BluetoothService.getActiveConnection()?.transport ||
    ...
    LogsService.add(
      ...
      'Call Initiated',
      `Calling ${device.name}${
        WEBRTC_LOCAL_NETWORK_ONLY ? ' over same Wi-Fi/hotspot' : ''
      }`,
      'OUTGOING',
    );
  ...
  _sendSignal: async payload => {
    ...
    const enrichedPayload = {
      ...
      signalNonce: payload?.signalNonce || generateSignalNonce(),
      senderIdentityId: payload?.senderIdentityId || localIdentity.id,
    };

    await BluetoothService.sendInvitePacket(signalingChannel, enrichedPayload);
    LogsService.add('signal', 'Signal Sent', enrichedPayload.type, 'TX');
  },
  ...
  _handlePeerConnectionState: state => {
    ...
    if (
      state === 'failed' &&
      (callState === CALL_STATES.ACTIVE || callState === CALL_STATES.OUTGOING)
    ) {
      clearDisconnectTimer();
    ...
    if (
      ...
      if (!disconnectTimer) {
        LogsService.add(
          'call',
          'Call Link Unstable',
          'Waiting before reconnect escalation',
          'WARN',
        );
