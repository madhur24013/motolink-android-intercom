...
export const ReconnectService = {
  start: async (
    savedDevice,
    {mode = 'reconnect', onAttempt, onPhase, onSuccess, onFailed} = {},
  ) => {
    ...
    shouldStop = false;
    currentAttempt = 0;
    TransportStateManager.setState(
      mode === 'auto'
        ? TRANSPORT_STATES.AUTO_CONNECTING
        : TRANSPORT_STATES.RECONNECTING,
    );
    LogsService.add(
      'reconnect',
      mode === 'auto' ? 'Auto Connect Started' : 'Reconnect Started',
      savedDevice.name || savedDevice.id,
      'START',
    );
    ...
    for (let attempt = 1; attempt <= RECONNECT_MAX_ATTEMPTS; attempt += 1) {
      ...
      try {
        ...
        await PairingService.validateSavedDeviceLink(channel, savedDevice);
        ...
        const connectionId =
          active?.transport === 'CLASSIC'
            ? active?.device?.address || active?.device?.id
            : active?.id;
        if (connectionId) {
          await BluetoothService.disconnect(connectionId).catch(() => null);
        }
        if (error?.rePairRequired) {
          LogsService.add(
            'reconnect',
            'Re-pair Required',
            error.message,
    ...
    isRunning = false;
    TransportStateManager.setState(
      shouldStop ? TRANSPORT_STATES.DISCONNECTED : TRANSPORT_STATES.FAILED,
    );
    LogsService.add(
      'reconnect',
      'Reconnect Failed',
  ...
  _findForReconnect: async savedDevice => {
    if (
      savedDevice.type === 'CLASSIC' ||
      savedDevice.preferredTransport === 'CLASSIC'
    ) {
      LogsService.add(
        'reconnect',
        'Classic Reconnect Path',
      ...
      BluetoothService.startScan(
        device => {
          if (
            ...
            device.address === savedDevice.address
          ) {
            foundDevice = device;
