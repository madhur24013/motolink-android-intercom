...
const safeNativeAudioCall = async (action, ...args) => {
  if (
    Platform.OS !== 'android' ||
    !MotoLinkAudioRoute ||
    typeof MotoLinkAudioRoute[action] !== 'function'
  ) {
    return false;
  }
  try {
...
const attachRemoteStream = (incomingStream, incomingTrack, onRemoteStream) => {
  ...
  if (!nextRemoteStream) {
    LogsService.add(
      ...
      'Track event did not contain a usable audio stream',
      'WARN',
    );
    return;
  }
  ...
  LogsService.add(
    'audio',
    'Remote Stream',
    `Incoming audio stream attached (${audioTracks.length} audio tracks)`,
    'RX',
  );
};
...
const attachLocalMedia = connection => {
  ...
  try {
    if (typeof connection.addTrack === 'function') {
      ...
      LogsService.add(
        'audio',
        'Local Tracks Added',
        `${localStream.getTracks().length} tracks via addTrack`,
        'TX',
      );
      return;
    }
  ...
  if (typeof connection.addStream === 'function') {
    ...
    LogsService.add(
      'audio',
      'Local Stream Added',
      `${localStream.getTracks().length} tracks via addStream`,
      'TX',
    );
  }
};
...
export const AudioService = {
  initMicrophone: async () => {
    ...
    LogsService.add(
      'audio',
      'Microphone Ready',
      `Local audio stream initialized ${AUDIO_SAMPLE_RATE}Hz ${AUDIO_CODEC} (${
        localStream.getAudioTracks().length
      } audio tracks, ${pttMode ? 'PTT' : 'FULL'})`,
      'READY',
    );
  ...
  createPeerConnection: (
    ...
    const rtcConfiguration = RuntimeConfigService.buildRtcConfiguration();
    ...
    LogsService.add(
      ...
      RuntimeConfigService.describeRtcConfiguration(rtcConfiguration),
      'PC',
    );
    ...
    peerConnection.onicecandidate = event => {
      ...
      if (!event.candidate) {
        LogsService.add(
          ...
          'Local ICE candidate collection completed',
          'ICE',
        );
      }
    };
  ...
  createOffer: async () => {
    ...
    const finalOffer =
      serializeSessionDescription(peerConnection?.localDescription) || offer;
    LogsService.add(
      'audio',
      'Offer Created',
  ...
  createAnswer: async offer => {
    ...
    await AudioService.waitForIceGathering();
    const finalAnswer =
      serializeSessionDescription(peerConnection?.localDescription) || answer;
    LogsService.add(
      'audio',
      'Answer Created',
  ...
  setAnswer: async answer => {
    ...
    LogsService.add(
      'audio',
      'Answer Applied',
      `Remote SDP answer set (${peerConnection?.signalingState || 'unknown'})`,
      'SET',
    );
  },
  ...
  addIceCandidate: async candidate => {
    if (!WEBRTC_TRICKLE_ICE) {
      LogsService.add(
        ...
        'Trickle ICE disabled in local network mode',
        'ICE',
      );
      return;
    }
    ...
    LogsService.add(
      ...
      `${candidate?.type || 'candidate'} ${candidate?.sdpMid || 'na'}`,
      'ICE',
    );
  },
  ...
  setFullDuplex: enabled => {
    ...
    LogsService.add(
      'audio',
      enabled ? 'Full Duplex' : 'PTT Mode',
      enabled ? 'Always transmitting' : 'Push to talk',
      'MODE',
    );
  },
  ...
  setSpeaker: useSpeaker => {
    ...
    LogsService.add(
      'audio',
      'Speaker Route',
      useSpeaker ? 'Speaker enabled' : 'Earpiece enabled',
      'ROUTE',
    );
  },
  ...
  waitForIceGathering: async () => {
    ...
    LogsService.add(
      'audio',
      'ICE Gathering Wait',
      `Waiting up to ${WEBRTC_ICE_GATHER_TIMEOUT_MS}ms for full local SDP`,
      'ICE',
    );
    let waiter = null;
    const result = await Promise.race([
      ...
      new Promise(resolve => {
        setTimeout(() => {
          if (waiter) {
            iceGatherWaiters = iceGatherWaiters.filter(
              entry => entry !== waiter,
            );
          }
          resolve('timeout');
